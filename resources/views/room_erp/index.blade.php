    ...
    <div x-show="extractModalOpen" 
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto"
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">Extract Room ERP from Downtime</h3>
            </div>
            <div class="p-6">
                ...
                <div x-show="!previewLoaded" class="mb-4">
                    ...
                    <button 
                        ...
                        @click="
                            ...
                            loading = true;
                            fetch('{{ route('room-erp.preview-from-downtime') }}', {
                                method: 'POST',
                                headers: {
                ...
                <template x-if="previewLoaded && previewData">
                    <div class="space-y-4">
                                ...
                                <table class="min-w-full text-sm">
                                    <thead class="bg-gray-200">
                                        <tr>
                                            ...
                                            <th class="px-3 py-2 text-left">Kode Room</th>
                                        ...
                                        <template x-if="previewData?.sample_data && previewData.sample_data.length > 0">
                                            <template x-for="(item, index) in previewData.sample_data" :key="index">
                                                <tr class="border-b">
                                                    ...
                                                    <td class="px-3 py-2" x-text="item.kode_room || '-'"></td>
                                                    <td class="px-3 py-2" x-text="item.name || ''"></td>
                                                    <td class="px-3 py-2" x-text="item.plant_name || ''"></td>
                                                    <td class="px-3 py-2" x-text="item.process_name || '-'"></td>
                                                    <td class="px-3 py-2" x-text="item.line_name || '-'"></td>
                                                </tr>
                                            </template>
                                        </template>
                                        <template x-if="!previewData?.sample_data || previewData.sample_data.length === 0">
                                            <tr>
                                                <td colspan="6" class="px-3 py-2 text-center text-gray-500">No sample data available</td>
                                            </tr>
                                        </template>
                                    </tbody>
                                </table>
                        ...
                        <form method="POST" action="{{ route('room-erp.extract-from-downtime') }}" id="extractForm">
                            @csrf
